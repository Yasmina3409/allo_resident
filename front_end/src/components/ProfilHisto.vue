<template>
  ...
  <div class=" list-group  taille_police style_font" v-for="annonce in annonces_user " :key="annonce.id">
    ...
    <div class="d-flex justify-content-end" v-if='user.user_id == profil_id.user_annonce || user.role == "superviseur"'>
    ...
    <div class="border-bottom  pr-7">
      <div class="d-flex justify-content-beteween mb-2 ml-2">
        <h6 class="fw-semi-bold text-wrap mr-3 ">Titre de :<br>l'{{ annonce.type_annonce
        }}&nbsp;&nbsp;
        </h6>
        <h2 class="fw-bold fs-5 taille_police">{{ annonce.titre }}</h2>
      </div>


      <div class=" d-flex justify-content-end mr-1">
        <h6 class="fw-semi-bold text-wrap mr-3 text-md  ml-1">Date de l'{{ annonce.type_annonce }}:
        </h6>
        <small class=" font-semibold
    text-color_allo_blue   text-decoration text-decoration-underline taille_police2">{{ annonce.date
    }}</small>
      </div>
      <div class=" d-flex justify-content-end mr-1 ">
        <h6 class="fw-semi-bold text-wrap mr-3 text-sm taille_police2"> Lieu de la publication :</h6>
        <small class=" font-semibold
    text-color_allo_blue   text-decoration text-decoration-underline taille_police2">{{ annonce.lieu
    }}</small>
      </div>
      <div class=" d-flex justify-content-end mr-2 mb-2">
        <h6 class="fw-semi-bold text-wrap mr-3  text-sm"> Type de publication :</h6>
        <small class=" font-semibold
    text-color_allo_blue   text-decoration text-decoration-underline">{{ annonce.type_annonce
    }}</small>
      </div>
    </div>
...
export default {
  ...
  methods: {
    async displayannonces() {
      ApiService2.get("getannonce").then(this.manageResults);
      ...
      annonces.forEach((annonce) => {
        ...
        if (this.user.user_id == this.profil_id.user_id) { this.id = this.user.user_id; }
        else { this.id = this.profil_id.user_annonce }
